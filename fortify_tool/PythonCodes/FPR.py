...
class FPR:
    ...
    def extractVulnerabilities(self):
        ...
        for vuln in vulns:
            ...
            if( isSinked):
                ...
                if tempRep[ len(tempRep) - 1].attrib['key'] == "SourceLocation":
                    sinkedVulnerability.filename = tempRep[ len(tempRep) - 1].attrib['path']
    ...
    def extractFileDictionary(self):
        with open('/tmp/xmlParser/src-archive/index.xml') as f:
            xmlstring = f.read()
        xmlstring = sub('\\sxmlns="[^"]+"', '', xmlstring, count=1)
        indexes = ET.fromstring(xmlstring)
        entries = indexes.findall('entry')
        for entry in entries:
            tempKey = entry.attrib['key']
            t = entry.text
            arr = t.split("/")
            tempValue = arr[1]
            self.fileDict[tempKey] = tempValue

    def writeFileNum(self):
        for k in self.getVulnerabilities():
            k.fileNum = self.fileDict[k.getFilename()]
